...
export async function GET(
  ...
  try {
    ...
    if (!rows || rows.length === 0) {
      // Vérifier si la première tâche a déjà été créée automatiquement
      const [existingTask]: any = await connection.execute(
        "SELECT COUNT(*) as count FROM tasks WHERE dao_id = ? AND titre = ?",
        [Number(id), "Résumé sommaire DAO et Création du drive"]
      );

      if (existingTask[0].count === 0) {
        // Créer uniquement les tâches par défaut si aucune tâche n'existe
