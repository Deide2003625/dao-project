// Vérification avec la bonne configuration de base de données
const mysql = require('mysql2/promise');

async function checkTasksTableCorrect() {
  console.log('=== VÉRIFICATION TABLE TASKS (CORRECT) ===');
  
  try {
    // Configuration de la base de données (même que dans lib/db.ts)
    const connection = await mysql.createPool({
      host: 'localhost',
      user: 'root',
      password: '',
      database: 'dao', // Nom correct de la base
      port: 3306,
    ...
    // Vérifier les DAOs disponibles
    const [daos] = await connection.execute(`
      SELECT id, numero FROM daos ORDER BY id LIMIT 5
    `);
    
    console.log('DAOs disponibles:');
    daos.forEach((dao, index) => {
      console.log(`  ${index + 1}. ID=${dao.id}, Numero=${dao.numero}`);
    });
    ...
    console.log('📋 Prochaine étape: Modifier l\'API pour charger les tâches depuis la base');
