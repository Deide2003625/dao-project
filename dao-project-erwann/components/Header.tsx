...
export default function Header() {
  ...
  const loadUserFromLocalStorage = () => {
    ...
    console.log('Chargement des données utilisateur depuis le localStorage:', storedUser);
    ...
    if (storedUser) {
      try {
        ...
        const userData = {
          id: parsedUser.id,
          username: parsedUser.username || parsedUser.email?.split('@')[0] || 'Utilisateur',
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === 'user' || e.key === null) {
        console.log('Changement détecté dans le localStorage, rechargement des données utilisateur');
        loadUserFromLocalStorage();
      }
    };
    ...
    const handleUserUpdated = (e: CustomEvent) => {
      console.log('Événement userUpdated reçu, mise à jour des données utilisateur');
      if (e.detail) {
        setUser({
          id: e.detail.id,
          username: e.detail.username || e.detail.email?.split('@')[0] || 'Utilisateur',
