...
export default function NewDaoPage() {
  ...
  const toggleMembre = (id:number) => {
    const s = membres.includes(String(id)) ? membres.filter(m => m !== String(id)) : [...membres, String(id)];
    setMembres(s);
  };
  ...
  const validate = () => {
    ...
    if (description.trim().length < 5) return "La description doit contenir au moins 5 caractères.";
    ...
    if (membres.length === 0) return "Au moins un membre d'équipe doit être sélectionné.";
    return null;
  };
      ...
      <div className="card">
        <div className="card-body">
          ...
          <form onSubmit={handleSubmit}>
            <div className="mb-3">
              <label className="form-label">Numéro de liste (automatique)</label>
              <input className="form-control" value={generatedNumber} readOnly />
              <div className="form-text">Numéro généré automatiquement en séquence</div>
            </div>

            <div className="mb-3">
              <label className="form-label">Date de dépôt *</label>
              <input type="date" className="form-control" value={dateDepot} onChange={e=>setDateDepot(e.target.value)} required />
            </div>

            <div className="mb-3">
              <label className="form-label">Objet du dossier *</label>
              <input className="form-control" value={objet} onChange={e=>setObjet(e.target.value)} required />
            </div>

            <div className="mb-3">
              <label className="form-label">Description détaillée du projet (minimum 5 caractères)</label>
              <textarea className="form-control" rows={4} value={description} onChange={e=>setDescription(e.target.value)} />
              <div className="form-text">{description.length}/5 caractères minimum</div>
            </div>

            <div className="mb-3">
              <label className="form-label">Référence *</label>
              <input className="form-control" value={reference} onChange={e=>setReference(e.target.value)} placeholder="ex: AMI-2025-SYSINFO" required />
            </div>

            <div className="mb-3">
              <label className="form-label">Autorité contractante *</label>
              <input className="form-control" value={autorite} onChange={e=>setAutorite(e.target.value)} required />
            </div>

            <div className="mb-3">
              <label className="form-label">Chef d'équipe *</label>
              <select className="form-select" value={chefEquipe} onChange={e=>setChefEquipe(e.target.value)} required>
            ...
            <div className="mb-3" style={{position:'relative'}} ref={membresRef}>
              <label className="form-label">Membres d'équipe *</label>
              <button ref={membresButtonRef} type="button" className="btn btn-light w-100 text-start" onClick={openMembres}>
                {membres.length > 0 ? `${membres.length} membre(s) sélectionné(s)` : 'Sélectionner des membres...'}
              </button>
              {membresOpen && (
                <div className="border p-2 bg-white" style={{position:'absolute', zIndex:50, maxHeight:240, overflow:'auto', width:'100%', ...(membresFlipUp ? {bottom: 'calc(100% + 8px)'} : {top: 'calc(100% + 8px)'})}}>
                  {users.length === 0 && <div className="text-muted">Aucun membre disponible</div>}
                  {users.map(u=> (
                    <label
                      key={u.id}
                      className="form-check d-flex align-items-center"
                      style={{cursor:'pointer', padding: '6px 8px', gap: '8px', fontSize: '0.95rem'}}
                    >
                      <input
                        ...
                        style={{width:18, height:18, minWidth:18, minHeight:18, margin:0, appearance:'checkbox'}}
                      />
                      <span className="form-check-label" style={{lineHeight: '1.2rem', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>{u.username}</span>
                    </label>
                  ))}
                </div>
              )}
            ...
            <div className="d-flex justify-content-end gap-2">
              <Link href="/dash/admin" className="btn btn-light">Annuler</Link>
              <button type="submit" className="btn btn-primary">Créer le DAO</button>
            </div>
          </form>
        </div>
      </div>
